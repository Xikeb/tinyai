...
constexpr static unsigned int maxSteps = 4*3600*100;
...
int main() {
	...
	unsigned int perfect_beings_count = 0;

	for(unsigned int best_fitness = 0; perfect_beings_count < (speciating_parameters.population * .7); pool.new_generation()) {
		unsigned int max_fitness = 0;
		unsigned int min_fitness = maxSteps;

		perfect_beings_count = 0;
		...
		for(auto &specie: pool.species) {
			for(auto &genome: specie.genomes) {
				...
				if (best_fitness < genome.fitness) {
					...
					// n.export_tofile("Balancer-HighScore.neat.nn");
				}

				perfect_beings_count += (genome.fitness == maxSteps);
			}
		}
