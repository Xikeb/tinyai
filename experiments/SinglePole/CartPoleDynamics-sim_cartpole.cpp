...
public:
  ...
  double Fx(double x_2, double x_3, double x_4){
    ...
    constexpr static auto toRad = PI/180.0;
    ...
    // broken into parts to help make it readable / a bit more clear
    double term1 = -1 * g * sin_3 * ml * cos_3;
    double term2 = -1 * length * (F + ml * sq(x_4) * sin_3);
    double alpha = (sq(cos_3) * ml) - length * (m_1 + m_2);
    double result = ((term1 + term2)/alpha) + mu * x_2; // TODO add friction here
  ...
  double Gx(double x_3, double x_4){
    ...
    // broken into parts to help make it readable / a bit more clear
    double term1 = -1 * (m_1 + m_2) * (-1 * g * sin_3);
    double term2 = cos_3 * (F + m_2 * length * sq(x_4) * sin_3);
    double alpha = ((sq(cos_3) * m_2 * length) - length * (m_1 + m_2));
    double result = ((term1 + term2)/alpha) + mu * x_4; // This removes energy from the system
