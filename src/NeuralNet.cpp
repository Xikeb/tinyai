...
#pragma GCC optimize("O3,Ofast,-funroll-loops,-march=native,-ffast-math,-ffinite-math-only,-funroll-loops,-fomit-frame-pointer")

#include "NeuralNet.hpp"

void ann::NeuralNet::evaluate_nonrecurrent(std::vector<double> const &input, std::vector<double> &output) {
    for (auto & node : nodes) {
    ...
    while (!s.empty()){
        ...
        if (nodes[nodeId].visited){
            double sum = 0.0;
            for (size_t i=0; i < nodes[nodeId].in_nodes.size(); i++)
                sum += nodes[nodes[nodeId].in_nodes[i].first].value * nodes[nodeId].in_nodes[i].second;
            nodes[nodeId].value = sigmoid(sum);
            s.pop();
        } else {
            nodes[nodeId].visited = true;

            for (size_t i=0; i < nodes[nodeId].in_nodes.size(); i++)
                if (not nodes[nodes[nodeId].in_nodes[i].first].visited) {
                    // if we haven't calculated value for this node
                    s.push(nodes[nodeId].in_nodes[i].first);
                }
        }
    }
...
void ann::NeuralNet::evaluate_recurrent(std::vector<double> const &input, std::vector<double> &output) {
...
void ann::NeuralNet::from_genome(neat::Genotype const &a) {
    ...
    nodes.clear();        nodes.reserve(a.network_info.functional_nodes);
    input_nodes.clear();  input_nodes.reserve(input_size);
    bias_nodes.clear();   bias_nodes.reserve(bias_size);
    output_nodes.clear(); output_nodes.reserve(output_size);
...
void ann::NeuralNet::evaluate(std::vector<double> const &input, std::vector<double> &output) {
    if (recurrent)
...
void ann::NeuralNet::import_fromfile(std::string const &filename) {
    std::ifstream o;
    o.open(filename);
    ...
    try {
        ...
        for (unsigned int i=0; i<neuron_number; i++){
            ...
            switch(type) {
                case 1:
                    input_nodes.push_back(i); break;
                case 2:
                    output_nodes.push_back(i); break;
                case 3:
                    bias_nodes.push_back(i); break;
                default:break;
            }
            // if (type == 1)
...
void ann::NeuralNet::export_tofile(std::string const &filename) {
    std::ofstream o;
    o.open(filename);
